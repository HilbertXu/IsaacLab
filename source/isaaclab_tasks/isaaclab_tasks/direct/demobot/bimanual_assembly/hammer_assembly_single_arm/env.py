...
class HammerAssemblyEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.enable_success_reset = self.cfg.enable_success_reset
        ...
        self.finger_tip_indices = list()
        for body_name in finger_tip_names:
            self.finger_tip_indices.append(self.robot.body_names.index(body_name))
        self.finger_tip_indices.sort()
        self.num_finger_tips = len(self.finger_tip_indices)
        joint_names = [n for n in self.robot.joint_names]
        self.joint_indices = [self.robot.joint_names.index(n) for n in joint_names]
        ...
        self.left_object_pos_tolerance = 0.01 * torch.ones(self.num_envs, dtype=torch.float, device=self.device)
        ...
        if self.debug:
            ...
            self.finger_markers = [VisualizationMarkers(frame_marker_cfg.replace(prim_path=f"/Visuals/finger_{i}")) for i in range(len(self.finger_tip_indices))]
    ...
    def _setup_action_chunks(self, action_buffer, right_offset, left_offset):
        ...
        ref_actions = torch.zeros((num_chunks, 500, 23), dtype=torch.float, device=self.device) # [num_chunks, num_steps, action_space]
    ...
    def _setup_scene(self):
        ...
        self.palm_frame = FrameTransformer(self.cfg.palm_config)

        self.scene.sensors["ee_frame"] = self.ee_frame
        self.scene.sensors["palm_frame"] = self.palm_frame
        ...
        cfg_ground = sim_utils.GroundPlaneCfg(color=(0.99, 0.83, 0.99))
        cfg_ground.func("/World/defaultGroundPlane", cfg_ground, translation=(0,0,-0.7))
    ...
    def _compute_action(self, pred_actions, ref_actions=None):
        ...
        if self.use_ref and ref_actions is not None:
            out = _compute_one_side(out, ref_actions=ref_actions, pred_actions=pred_actions, indices=self.joint_indices, control_mode=self.control_mode)
        else:
            out = _compute_one_side(out, ref_actions=None, pred_actions=pred_actions, indices=self.joint_indices, control_mode=self.control_mode)
    ...
    def _compute_intermediate_values(self):
        ...
        self.fingertip_pos = self.robot.data.body_pos_w[:, self.finger_tip_indices].clone()
        self.fingertip_pos -= self.scene.env_origins.repeat((1, self.num_finger_tips)).reshape(
            self.num_envs, self.num_finger_tips, 3
        )
        self.fingertip_quat = self.robot.data.body_quat_w[:, self.finger_tip_indices].clone()
        self.fingertip_velocities = self.robot.data.body_vel_w[:, self.finger_tip_indices].clone()
        
        self.palm_pos = self.palm_frame.data.target_pos_source[..., 0, :]
        self.ee_pos = self.ee_frame.data.target_pos_source[..., 0, :]
        self.ee_quat = self.ee_frame.data.target_quat_source[..., 0, :]
        ...
        # For reaching stage
        self.right_ee2o_dist = torch.clamp(torch.norm(self.ee_pos - self.right_object_pos, p=2, dim=-1), min=-9.0, max=9.0)
        self.left_ee2o_dist = torch.clamp(torch.norm(self.ee_pos - self.left_object_pos, p=2, dim=-1), min=-9.0, max=9.0)
        
        self.right_h2o_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.right_object_grasp_keypoint_cur).min(dim=-1).values).max(dim=-1).values, min=-9.0, max=9.0)
        self.left_h2o_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.left_object_grasp_keypoint_cur).min(dim=-1).values).max(dim=-1).values, min=-9.0, max=9.0)

        self.right_h2o_min_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.right_object_grasp_keypoint_cur).min(dim=-1).values).min(dim=-1).values, min=-9.0, max=9.0)
        self.left_h2o_min_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.left_object_grasp_keypoint_cur).min(dim=-1).values).min(dim=-1).values, min=-9.0, max=9.0)
        ...
        # print('right_ee2o_dist', self.right_ee2o_dist)
        # print('left_ee2o_dist', self.left_ee2o_dist)
        # print('right_h2o_dist', self.right_h2o_dist)
        # print('left_h2o_dist', self.left_h2o_dist)
        # print('right_h2o_min_dist', self.right_h2o_min_dist)
        # print('left_h2o_min_dist', self.left_h2o_min_dist)
        ...
        self.right_object_reached = torch.where(
            self.successes == self.right_init_id,
            (self.right_ee2o_dist < 0.04) & (self.ee_pos[:, 2] - self.right_object_pos[:, 2] <= 0.03),
            (self.right_ee2o_dist < 0.07)
        )
        self.left_object_reached = torch.where(
            self.successes == self.left_init_id,
            (self.left_ee2o_dist < 0.04) & (self.ee_pos[:, 2] - self.left_object_pos[:, 2] <= 0.03),
        ...
        self.object_dropped = (self.right_object_pos[:, 2] < 0.1) | (self.left_object_pos[:, 2] < 0.1)
        ...
        self.action_penalty = 0.001 *torch.norm(self.pred_actions[:, self.joint_indices], p=2, dim=-1)
    ...
    def _get_observations(self) -> dict:
        if self.cfg.asymmetric_obs:
            self.fingertip_force_sensors = self.robot.root_physx_view.get_link_incoming_joint_force()[
    ...
    def compute_reduced_observations(self):
        ...
        right_base_obs = [
            # robot joint pos & vel
            unscale(self.robot_dof_pos, self.robot_dof_lower_limits, self.robot_dof_upper_limits)[:, self.joint_indices],
            (self.cfg.vel_obs_scale * self.robot_dof_vel)[:, self.joint_indices],
            # hand to object
            self.ee_pos - self.right_object_pos, # right ee to right object translation
            (self.fingertip_pos - self.right_object_pos.unsqueeze(1)).view(self.num_envs, self.num_finger_tips * 3),
        ]
        right_ref_act_obs = [
            self.ref_action_norm[:, self.joint_indices]
        ]
        ...
        left_base_obs = [
            ...
            unscale(self.robot_dof_pos, self.robot_dof_lower_limits, self.robot_dof_upper_limits)[:, self.joint_indices],
            (self.cfg.vel_obs_scale * self.robot_dof_vel)[:, self.joint_indices],
            self.ee_pos - self.left_object_pos, # left ee to left object translation
            (self.fingertip_pos - self.left_object_pos.unsqueeze(1)).view(self.num_envs, self.num_finger_tips * 3),
        ]
        left_ref_act_obs = [
            self.ref_action_norm[:, self.joint_indices]
        ]
    ...
    def compute_full_state(self):
        ...
        right_base_states = [
            ...
            self.ee_pos - self.right_object_pos, # right ee to right object translation
            # right finger tips
            self.fingertip_pos.view(self.num_envs, self.num_finger_tips * 3),
            self.fingertip_quat.view(self.num_envs, self.num_finger_tips * 4),
            self.fingertip_velocities.view(self.num_envs, self.num_finger_tips * 6),
            self.cfg.force_torque_obs_scale * self.fingertip_force_sensors.view(self.num_envs, self.num_finger_tips * 6),
            (self.fingertip_pos - self.right_object_pos.unsqueeze(1)).view(self.num_envs, self.num_finger_tips * 3),
        ]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.debug == 2:
            self.palm_marker.visualize(self.ee_pos+self.scene.env_origins, self.ee_quat)
            ...
            for idx, marker in enumerate(self.finger_markers):
                marker.visualize(self.fingertip_pos[:, idx, :] + self.scene.env_origins, self.fingertip_quat[:, idx, :])
        ...
        # self.reset_goal_buf = (self.ref_chunk_step_idx[:, 1] >= self.ref_chunk_max_steps[self.ref_chunk_step_idx[:, 0]]) #NOTE test only
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        right_h2o_min_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.right_object_grasp_keypoint_cur).min(dim=-1).values).min(dim=-1).values, min=-9.0, max=9.0)
        left_h2o_min_dist = torch.clamp((torch.cdist(self.fingertip_pos, self.left_object_grasp_keypoint_cur).min(dim=-1).values).min(dim=-1).values, min=-9.0, max=9.0)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        def _reset_object(obj, pos_noise, rot_noise_quat_z, add_noise=False):
        ...
        _reset_object(self.right_object, pos_noise=pos_noise, rot_noise_quat_z=rot_noise_quat_z, add_noise=True)
        _reset_object(self.left_object, pos_noise=pos_noise, rot_noise_quat_z=rot_noise_quat_z, add_noise=True)
    ...
    def _reset_envs(
        ...
        if last_succ_candidate_ids.numel() > 0:
            num_to_sample = int(math.floor(last_succ_candidate_ids.numel() * self.reset_to_last_success_ratio))
            if num_to_sample > 0:
    ...
    def _reset_target_pose(self, env_ids):
        ...
        # @TODO
        # reset the cur operation side
        switch_env_ids = env_ids[self.successes[env_ids] == self.switch_idx]
        if len(switch_env_ids) > 0:
            self.cur_active_side[switch_env_ids] = ~self.cur_active_side[switch_env_ids]
