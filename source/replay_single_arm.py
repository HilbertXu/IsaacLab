...
parser.add_argument("--robot", type=str, default="franka_allegro_left", help="Name of the robot_right.")
...
parser.add_argument("--seq_name", type=str, default="realsense/single_arm_v2_fps30")
...
##
from isaaclab_assets import FRANKA_ALLEGRO_BIMANUAL_HIGH_PD_CFG, FRANKA_ALLEGRO_LEFT_HIGH_PD_CFG   # isort:skip
...
class ReplaySceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    if args_cli.robot == "franka_allegro_left":
        print("Load franka allegro")
        robot = FRANKA_ALLEGRO_LEFT_HIGH_PD_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
class DemoReplay(object):
    def __init__(self, scene_cfg, args_cli):
        ...
        self.robot_name = args_cli.robot
    ...
    def setup_sim(self, args_cli):
        ...
        if args_cli.robot == "franka_allegro_left":
            ...
            self.hand_entity_cfg = SceneEntityCfg(
                "robot", 
                joint_names=[
        ...
        elif args_cli.robot == "franka_allegro_right":
            self.arm_entity_cfg = SceneEntityCfg(
                "robot", 
                joint_names=[
        ...
        self.hand_entity_cfg.resolve(self.scene)
        self.robot_entity_cfg.resolve(self.scene)

        self.arm_joint_ids = self.arm_entity_cfg.joint_ids
        self.arm_joint_names = self.arm_entity_cfg.joint_names
        self.hand_joint_ids = self.hand_entity_cfg.joint_ids
        self.robot_joint_ids = self.robot_entity_cfg.joint_ids
        self.robot_joint_names = self.robot_entity_cfg.joint_names

        # Obtain the frame index of the end-effector
        # For a fixed base robot, the frame index is one less than the body index. This is because
        # the root body is not included in the returned Jacobians.
        # In this replay script, we only setup on body in the robot cfg
        self.ee_body_id = self.arm_entity_cfg.body_ids[1]
        self.root_body_id = self.arm_entity_cfg.body_ids[0]
        ...
        self.robot.reset()
        self.robot_base_offset = (self.robot.data.body_state_w[:, self.arm_entity_cfg.body_ids[0], :3] - self.scene.env_origins)[0, :]
        ...
        if args_cli.debug:
            ...
            self.ee_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_r_current"))
            self.goal_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_r_goal"))
            self.ee_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_l_current"))
            self.goal_marker = VisualizationMarkers(frame_marker_cfg.replace(prim_path="/Visuals/ee_l_goal"))
    ...
    def read_data(self, args_cli):
        ...
        self.right_ee_offset = np.asarray([0.65, -0.0, 0.10])
        self.left_ee_offset = np.asarray([0.65, -0.0, 0.10])
        self.retarget_data = {
            'ref_ee_pose': None,
            'ref_hand_qpos': None,
            'ref_obj_pose.right': None,
            'ref_obj_pose.left': None,
        }
        object_poses = []
        for key in retarget_bimanual.keys():
            retarget = retarget_bimanual[key].item()
            if key in self.robot_name:
                ...
                self.retarget_data['ref_ee_pose'] = ref_ee_pose
                self.retarget_data['ref_hand_qpos'] = ref_hand_qpos
            ...
            self.retarget_data[f'ref_obj_pose.{key}'] = object_poses
    ...
    def calculate_qpos_ik(self):
        jacobian = self.robot.root_physx_view.get_jacobians()[:, self.ee_jacobi_idx, :, self.arm_joint_ids]
        ee_pose_w = self.robot.data.body_state_w[:, self.ee_body_id, :7]
        root_pose_w = self.robot.data.body_state_w[:, self.root_body_id, :7]
        ee_pos_b, ee_quat_b = subtract_frame_transforms(
                    root_pose_w[:, 0:3], root_pose_w[:, 3:7], ee_pose_w[:, 0:3], ee_pose_w[:, 3:7]
                )

        position_error, axis_angle_error = compute_pose_error(
                    ee_pos_b, ee_quat_b, self.ik_commands[:, :3], self.ik_commands[:, 3:7], rot_error_type="axis_angle"
                )
        ...
        if self.debug:
            ...
            self.goal_marker.visualize(
                self.ik_commands[:, 0:3] + self.robot_base_offset + self.scene.env_origins, 
    ...
    def move_to_initial_waypoint(
        self, 
        goal_ee_pose, right_object_pose, left_object_pose,
    ):
        
        self.diff_ik_controller.reset()
        self.ik_commands[:] = goal_ee_pose
        self.ik_commands[:, :3] -= self.robot_base_offset
        ...
        while not success:
            ...
            joint_pos_des, reached = self.calculate_qpos_ik()
            self.joint_pos[:, self.arm_joint_ids] = joint_pos_des
            if reached:
                init_robot_qpos = joint_pos_des[0, :].cpu().numpy()
                init_robot_qpos_dict = {}
                init_robot_qpos_dict.update(
                    {k:v for (k, v) in zip(self.arm_joint_names, init_robot_qpos)}
                )

                init_robot_qpos = self.robot.data.default_joint_pos.clone()[0, :]
                init_robot_qpos[self.arm_joint_ids] = joint_pos_des[0, :]
                success = True
            else:
    ...
    def move_to_next_waypoint(
        ...
        hand_qpos = self.joint_pos[:, self.hand_joint_ids]
        hand_qpos[:] = hand_ref_qpos * 0.95
        ...
        while not success:
            ...
            joint_pos_des, reached = self.calculate_qpos_ik()
            self.joint_pos[:, self.arm_joint_ids] = joint_pos_des
            self.joint_pos[:, self.hand_joint_ids] = hand_ref_qpos
    ...
    def replay(self):
        ...
        while simulation_app.is_running():
            ...
            init_robot_qpos_dict, init_robot_qpos = self.move_to_initial_waypoint(
                goal_ee_pose=self.retarget_data['ref_ee_pose'][getattr(self, f"{self.order[0]}_keyframe_indices")[0], :], 
            ...
            for waypoint_idx in range(start_idx, self.num_waypoints):
                ...
                cur_goal_object_pose_rw, cur_goal_object_pose_rb = self.transform_object_pose_cam_to_world(
                    self.retarget_data['ref_obj_pose.right'][waypoint_idx, :],
                    offset=self.right_offset
                )
                cur_goal_object_pose_lw, cur_goal_object_pose_lb = self.transform_object_pose_cam_to_world(
                    self.retarget_data['ref_obj_pose.left'][waypoint_idx, :],
                    offset=self.left_offset
                )
                ...
                goal_ee_pose=self.retarget_data['ref_ee_pose'][waypoint_idx, :]
                hand_ref_qpos=self.retarget_data['ref_hand_qpos'][waypoint_idx, :]
                right_object_pose=cur_goal_object_pose_rw
                left_object_pose=cur_goal_object_pose_lw

                if last_ee_pose is None:
                    last_ee_pose = goal_ee_pose
                
                move_dist = torch.norm(goal_ee_pose[:3] - last_ee_pose[:3], dim=-1)
                # remove outliers
                if move_dist > 0.4 and waypoint_idx != getattr(self, f"{self.order[-1]}_keyframe_indices")[0]:
                    continue

                actions, pre_grasp_flag, pre_grasp_rot = self.move_to_next_waypoint(
                    goal_ee_pose=goal_ee_pose, 
                    hand_ref_qpos=hand_ref_qpos, 
            ...
            if not self.repeat:
                for key in trajectorys["action_chunks"].keys():
                    ...
                    print(key, trajectorys["action_chunks"][key]['qpos'].shape)
